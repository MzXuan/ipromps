...
def main():
    # muP = np.ones((10,1))
    # muq = np.ones((10,1))
    # sigmaP = 0.5*np.eye(10) + 0.5*np.ones((10,1))
    # sigmaq = 0.5*np.eye(10)+ 0.5*np.ones((10,1))

    # gau_kl(muP , sigmaP, muq, sigmaq)    
    task_id = 1
    test_index = 20
    ...
    for fig_idx,obs_ratio in enumerate(obs_ratio_time):
        ...
        for task_idx, ipromps_idx in enumerate(ipromps_set):
            ...
            # KL_fuc(mean_traj,sigma_traj,mean_merge_traj,sigma_merge_traj)  
